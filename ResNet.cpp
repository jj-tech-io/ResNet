...
int main()
{
    ...
    const std::string imageFile = "C:\\Users\\joeli\\OneDrive\\Documents\\GitHub\\cpp_projects\\ResNet\\Assets\\dog.png";
    const std::string labelFile = "C:\\Users\\joeli\\OneDrive\\Documents\\GitHub\\cpp_projects\\ResNet\\Assets\\imagenet_classes.txt";
    auto modelPath = L"C:\\Users\\joeli\\OneDrive\\Documents\\GitHub\\cpp_projects\\ResNet\\Assets\\resnet50v2.onnx";
    ...
    //display image with label probabilities use opencv
    ...
    // add label and probability on the image
    for (size_t i = 0; i < 5; ++i) {
        const auto& result = indexValuePairs[i];
        std::stringstream ss;
        ss << labels[result.first] << " " << result.second;
        cv::putText(image, ss.str(), cv::Point(10, 30 + i * 30), cv::FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(255, 0, 0), 2);
    }
    //resize to fit the screen
    cv ::resize(image, image, cv::Size(1600, 900));
    // show the image with label and probability
    cv::imshow("Image with label and probability", image);
